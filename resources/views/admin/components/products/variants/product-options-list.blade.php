...
>
  @foreach($items as $itemIndex => $item)
    <div wire:key="option_{{ $itemIndex }}" x-sortable-option-item="option_{{ $itemIndex }}">
      <div class="grid grid-cols-2 space-x-4">
        <div>
            <div>
              <x-filament-forms::field-wrapper.label class="ml-7">
                {{ __('admin::components.product-options-list.name.label') }}
              ...
              <div class="flex w-full space-x-2 mt-1 items-start">
                ...
                >
                  <x-filament::icon alias="store::reorder" class="w-5 h-5" />
                </div>
                <div class="grow">
                  ...
                  >
                    {{ __(
                        !$item['readonly'] ?
                            'admin::components.product-options-list.delete-option.label' :
                            'admin::components.product-options-list.remove-shared-option.label'
                    ) }}
                  </button>
                </div>
              </div>
        ...
        <div class="space-y-1">
          <x-filament-forms::field-wrapper.label>
            {{ __('admin::components.product-options-list.values.label') }}
          </x-filament-forms::field-wrapper.label>
          <div wire:key="option_values_{{ $itemIndex }}">
            <x-admin::products.variants.product-option-list-values
  ...
  <x-filament::button color="gray" size="sm" type="button" wire:click.prevent="addRestrictedOption">
    {{ __('admin::components.product-options-list.add-option.label') }}
  </x-filament::button>
</div>