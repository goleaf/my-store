    ...
    <div class="space-y-4">
        ...
        <div class="fi-ta-header flex flex-col gap-3 p-4 sm:px-6 sm:flex-row sm:items-center">
          <div class="grid gap-y-1">
            <h3 class="fi-ta-header-heading text-base font-semibold leading-6 text-gray-950 dark:text-white">
              {{ __('admin::productoption.widgets.product-options.options-table.title') }}
            </h3>
          </div>
          <div class="fi-ta-actions flex shrink-0 items-center gap-3 flex-wrap justify-start sm:ms-auto">
            <x-filament::button type="button" wire:click="$set('configuringOptions', true)">
              {{ __('admin::productoption.widgets.product-options.options-table.configure-options.label') }}
            </x-filament::button>
          </div>
        </div>
                ...
                <tr class="bg-gray-50 dark:bg-white/5">
                  <x-filament-tables::header-cell>
                      <span class="fi-ta-header-cell-label text-sm font-semibold text-gray-950 dark:text-white">
                        {{ __('admin::productoption.widgets.product-options.options-table.table.option.label') }}
                      </span>
                  </x-filament-tables::header-cell>
                  <x-filament-tables::header-cell>
                      <span class="fi-ta-header-cell-label text-sm font-semibold text-gray-950 dark:text-white">
                        {{ __('admin::productoption.widgets.product-options.options-table.table.values.label') }}
                      </span>
                  </x-filament-tables::header-cell>
                </tr>
              </thead>
      ...
      <div class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:divide-white/10 dark:bg-gray-900 dark:ring-white/10">
        <div class="fi-ta-header flex flex-col gap-3 p-4 sm:px-6 sm:flex-row sm:items-center">
          <div class="grid gap-y-1">
            <h3 class="fi-ta-header-heading text-base font-semibold leading-6 text-gray-950 dark:text-white">
              {{ __('admin::productoption.widgets.product-options.variants-table.title') }}
            </h3>
          </div>
        </div>
        <div class="fi-ta-content divide-y divide-gray-200 overflow-x-auto dark:divide-white/10 dark:border-t-white/10">
          @if(count($this->variants))
                ...
                <thead class="divide-y divide-gray-200 dark:divide-white/5">
                  <tr class="bg-gray-50 dark:bg-white/5">
                    ...
                    <x-filament-tables::header-cell class="fi-ta-header-cell px-3 py-3.5 sm:first-of-type:ps-6 sm:last-of-type:pe-6">
                      <span class="fi-ta-header-cell-label text-sm font-semibold text-gray-950 dark:text-white">
                        {{ __('admin::productoption.widgets.product-options.variants-table.table.option.label') }}
                      </span>
                    </x-filament-tables::header-cell>
                    <x-filament-tables::header-cell>
                      <span class="fi-ta-header-cell-label text-sm font-semibold text-gray-950 dark:text-white">
                        {{ __('admin::productoption.widgets.product-options.variants-table.table.sku.label') }}
                      </span>
                    </x-filament-tables::header-cell>
                    <x-filament-tables::header-cell>
                      <span class="fi-ta-header-cell-label text-sm font-semibold text-gray-950 dark:text-white">
                        {{ __('admin::productoption.widgets.product-options.variants-table.table.price.label') }}
                      </span>
                    </x-filament-tables::header-cell>
                    <x-filament-tables::header-cell>
                      <span class="fi-ta-header-cell-label text-sm font-semibold text-gray-950 dark:text-white">
                        {{ __('admin::productoption.widgets.product-options.variants-table.table.stock.label') }}
                      </span>
                    </x-filament-tables::header-cell>
                    <x-filament-tables::header-cell>
                    </x-filament-tables::header-cell>
                        ...
                        <div class="fi-ta-text grid w-full gap-y-1 px-3 py-4">
                          @if(!$permutation['variant_id'])
                            <x-filament::badge color="info">
                              {{ __('admin::productoption.widgets.product-options.variants-table.table.new.label') }}
                            </x-filament::badge>
                          @endif
                        </div>
                      </x-filament-tables::cell>
                      ...
                      <div class="flex items-center space-x-2">
                        @if($permutation['variant_id'])
                          <x-filament::link :href="$this->getVariantLink($permutation['variant_id'])">
                            {{ __('admin::productoption.widgets.product-options.variants-table.actions.edit.label') }}
                          </x-filament::link>
                        @endif
                        <button type="button" wire:click="removeVariant('{{ $permutationIndex }}')" class="text-red-500 font-semibold text-sm hover:underline">
                          {{ __('admin::productoption.widgets.product-options.variants-table.actions.delete.label') }}
                        </button>
                      </div>
                    </x-filament-tables::cell>
            ...
            @else
              <x-filament-tables::empty-state :heading="__('admin::productoption.widgets.product-options.variants-table.empty.heading')" icon="lucide-shapes"></x-filament-tables::empty-state>
            @endif
        </div>
      </div>
    </div>
    ...
    <div class="space-y-4">
      <div class="text-right">
        ...
        <x-filament::button color="gray" wire:click="addRestrictedOption">
          {{ __('admin::productoption.widgets.product-options.actions.add-restricted-option.label') }}
        </x-filament::button>
      ...
      @if(!count($this->configuredOptions))
        <div wire:key="product_options">
          <x-filament-tables::empty-state
            :heading="__('admin::productoption.widgets.product-options.options-list.empty.heading')"
            :description="__('admin::productoption.widgets.product-options.options-list.empty.description')"
            icon="lucide-shapes"
          ></x-filament-tables::empty-state>
        </div>
      @else
        <div>
          <x-admin::products.variants.product-options-list
      ...
      <div class="flex space-x-2 border-t dark:border-white/10 pt-4">
        <x-filament::button type="button" wire:click="updateConfiguredOptions">
          {{ __('admin::productoption.widgets.product-options.actions.save-options.label') }}
        </x-filament::button>
        <x-filament::button color="gray" wire:click="cancelOptionConfiguring">
          {{ __('admin::productoption.widgets.product-options.actions.cancel.label') }}
        </x-filament::button>
      </div>
    </div>
