...
trait HasModelExtending
{
    ...
    public static function isStoreInstance(): bool
    {
    ...
    protected function fireModelEvent($event, $halt = true): mixed
    {
        ...
        $storeClass = str_replace('Contracts\\', '', ModelManifest::guessContractClass(static::class));

        if ($storeClass == static::class) {
            return $result;
        }

        return static::$dispatcher->{($halt ? 'until' : 'dispatch')}(
            "eloquent.{$event}: ".$storeClass, $this
        );
    }
}
