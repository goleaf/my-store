...
class CartLineQuantity extends BaseValidator
{
    ...
    public function validate(): bool
    {
        ...
        if ($quantity < 1) {
            $this->fail(
                'cart',
                __('store::exceptions.invalid_cart_line_quantity', [
                    'quantity' => $quantity,
                ])
            );
        }
        ...
        if ($quantity > 1000000) {
            $this->fail(
                'cart',
                __('store::exceptions.maximum_cart_line_quantity', [
                    'quantity' => 1000000,
                ])
            );
        }
        ...
        if ($purchasable && $quantity < $purchasable->min_quantity) {
            $this->fail(
                'cart',
                __('store::exceptions.minimum_quantity', [
        ...
        if ($purchasable && ($quantity % ($purchasable->quantity_increment ?? 1)) !== 0) {
            $this->fail(
                'cart',
                __('store::exceptions.quantity_increment', [
                    'quantity' => $quantity,
