...
trait DisplaysOrderAddresses
{
            ...
            Forms\Components\Grid::make()
                ->schema([
                    ...
                    Forms\Components\TextInput::make('last_name')
                        ->label(__('admin::order.form.address.last_name.label'))
                        ->autocomplete(false)
                        ->maxLength(255),
                ]),
            Forms\Components\TextInput::make('company_name')
                ->label(__('admin::order.form.address.company_name.label'))
                ->autocomplete(false)
                ->maxLength(255),
            ...
            Forms\Components\Grid::make()
                ->schema([
                    ...
                    Forms\Components\TextInput::make('contact_email')
                        ->label(__('admin::order.form.address.contact_email.label'))
                        ->autocomplete(false)
                        ->maxLength(255),
                ]),
            Forms\Components\TextInput::make('line_one')
                ->label(__('admin::order.form.address.line_one.label'))
                ->autocomplete(false)
                ->maxLength(255)
                ->required(),
            Forms\Components\Grid::make()
                ->schema([
                    ...
                    Forms\Components\TextInput::make('line_three')
                        ->label(__('admin::order.form.address.line_three.label'))
                        ->autocomplete(false)
                        ->maxLength(255),
                ]),
            Forms\Components\Grid::make(3)
                ->schema([
                    ...
                    Forms\Components\TextInput::make('state')
                        ->label(__('admin::order.form.address.state.label'))
                    ...
                    Forms\Components\TextInput::make('postcode')
                        ->label(__('admin::order.form.address.postcode.label'))
                        ->autocomplete(false)
                        ->maxLength(255),
                ]),
            Forms\Components\Select::make('country_id')
                ->label(__('admin::order.form.address.country_id.label'))
        ...
        return Infolists\Components\Section::make(__("admin::order.infolist.{$type}_address.label"))
            ...
            ->schema(fn ($record) => $sameAsShipping($record) ? [
                Infolists\Components\TextEntry::make('billing_matches_shipping')
                    ...
                    ->getStateUsing(fn () => __('admin::order.infolist.billing_matches_shipping.label')),
                ...
                Infolists\Components\TextEntry::make($type.'_address')
                    ...
                    ->getStateUsing(function ($record) use ($getAddress) {
                        ...
                        if ($address?->id ?? false) {
                            ...
                            return __('admin::order.infolist.address_not_set.label');
                        }
                    }),
    ...
    public static function getEditAddressAction(string $type): Action
    {
        return Action::make("edit_{$type}_address")
            ->modalHeading(__("admin::order.infolist.{$type}_address.label"))
            ->modalWidth('2xl')
            ->label(__('admin::order.action.edit_address.label'))
            ->button()
            ...
            ->action(function (Action $action, $record, $data) use ($type) {
                ...
                if (blank($addressType)) {
                    $action->failureNotificationTitle(__('admin::order.action.edit_address.notification.error'));
                    $action->failure();
                ...
                $action->successNotificationTitle(__("admin::order.action.edit_address.notification.{$type}_address.saved"));
