...
class Cart extends BaseModel implements Contracts\Cart
{
    ...
    public function calculate(bool $force = false): Cart
    {
        ...
        $cart = app(Pipeline::class)
            ->send($this)
            ->through(
                config('store.cart.pipelines.cart', [
                    Calculate::class,
                ])
            )->thenReturn();
    ...
    {
        foreach (config('store.cart.validators.add_to_cart', []) as $action) {
        ...
        return app(
            config('store.cart.actions.add_to_cart', AddOrUpdatePurchasable::class)
    ...
    {
        foreach (config('store.cart.validators.remove_from_cart', []) as $action) {
            app($action)->using(
                cart: $this,
        ...
        return app(
            config('store.cart.actions.remove_from_cart', RemovePurchasable::class)
    ...
    {
        foreach (config('store.cart.validators.update_cart_line', []) as $action) {
            app($action)->using(
                cart: $this,
                cartLineId: $cartLineId,
                quantity: $quantity,
        ...
        return app(
            config('store.cart.actions.update_cart_line', UpdateCartLine::class)
    ...
    {
        foreach (config('store.cart.validators.add_address', []) as $action) {
            app($action)->using(
                cart: $this,
                address: $address,
                type: $type,
        ...
        return app(
            config('store.cart.actions.add_address', AddAddress::class)
    ...
    {
        foreach (config('store.cart.validators.set_shipping_option', []) as $action) {
            app($action)->using(
                cart: $this,
        ...
        return app(
            config('store.cart.actions.set_shipping_option', SetShippingOption::class)
        )->execute($this, $option)
            ->then(fn () => $refresh ? $this->refresh()->recalculate() : $this);
    }
    ...
    public function createOrder(
        ...
        foreach (config('store.cart.validators.order_create', [
        ...
        return app(
            config('store.cart.actions.order_create', CreateOrder::class)
        )->execute(
            $cart,
            $allowMultipleOrders,
            $orderIdToUpdate
    ...
    {
        $generator = config('store.cart.fingerprint_generator', GenerateFingerprint::class);
