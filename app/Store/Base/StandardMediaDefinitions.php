...
class StandardMediaDefinitions implements MediaDefinitionsInterface
{
    ...
    {
        $fallbackUrl = config('store.media.fallback.url');
        $fallbackPath = config('store.media.fallback.path');

        // Reset to avoid duplication
        $model->mediaCollections = [];

        $collection = $model->addMediaCollection(
            config('store.media.collection')
        );
    ...
    {
        return [
            'images' => __('store::base.standard-media-definitions.collection-titles.images'),
        ];
    }
