...
namespace App\Store\Console\Commands;
...
class ScoutIndexerCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'store:search:index
    ...
    public function handle()
    {
        ...
        if (config('scout.driver') == 'meilisearch') {
            if (! $this->getApplication()->has('store:meilisearch:setup')) {
                $this->components->warn('Laravel scout config to use meilisearch driver, you may want to install and setup meilisearch package.');
            }
        }

        // Return searchable models from config
        $searchables = config('store.search.models', []);
