...
class CollectionConditionRelationManager extends BaseRelationManager
{
    ...
    {
        return __('admin::discount.relationmanagers.collection_conditions.title');
    }
    ...
    public function getDefaultTable(Table $table): Table
    {
        ...
        return $table
            ->heading(
                __('admin::discount.relationmanagers.collection_conditions.title')
            )
            ->description(
                __('admin::discount.relationmanagers.collection_conditions.description')
            )
            ->paginated(false)
            ->modifyQueryUsing(
            ...
            ->headerActions([
                Tables\Actions\AttachAction::make()->form(fn (Tables\Actions\AttachAction $action): array => [
                    ...
                    ->preloadRecordSelect()
                    ->label(
                        __('admin::discount.relationmanagers.collection_conditions.actions.attach.label')
                    ),
            ])->columns([
                Tables\Columns\TextColumn::make('id')
                    ->label(
                        __('admin::discount.relationmanagers.collection_conditions.table.name.label')
                    )
                    ->formatStateUsing(
