...
namespace App\Admin\Filament\Resources\CollectionGroupResource\Widgets;
...
use App\Admin\Support\Actions\Collections\CreateChildCollection;
use App\Admin\Support\Actions\Collections\CreateRootCollection;
use App\Admin\Support\Actions\Collections\DeleteCollection;
use App\Admin\Support\Actions\Collections\MoveCollection;
...
class CollectionTreeView extends Widget implements HasActions, HasForms
{
    ...
    public function sort($movedId, $siblingId, $direction = 'after')
    {
        ...
        Notification::make()
            ->title(
                __('admin::components.collection-tree-view.notifications.collections-reordered.success')
            )
            ->success()
            ->send();
    }
    ...
    public function toggleChildren($nodeId, $keepOpen = false)
    {
        ...
        if (! $index) {
            Notification::make()
                ->title(
                    __('admin::components.collection-tree-view.notifications.node-expanded.danger')
                )
                ->danger()
                ->send();
    ...
    public function addChildCollectionAction()
    {
        return CreateChildCollection::make('addChildCollection')
            ->icon(
                fn () => FilamentIcon::resolve('store::sub-collection')
            )->after(
    ...
    public function moveAction()
    {
        return MoveCollection::make('move')
            ->icon(
                fn () => FilamentIcon::resolve('store::move-collection')
            )->form([
                Forms\Components\Select::make('target_id')
                    ->label(
                        __('admin::components.collection-tree-view.actions.move.form.target_id.label')
                    )
